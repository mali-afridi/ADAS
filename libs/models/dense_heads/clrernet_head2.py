...
class CLRerHead(nn.Module):
    ...
    def loss(self, out_dict, img_metas):
        ...
        if self.loss_seg:
            if(self.evaluating):
                return 0
            tgt_masks = np.array([t["gt_masks"][0] for t in img_metas])
            # import pdb; pdb.set_trace()
    ...
    def get_lanes(self, pred_dict, as_lanes=True):
        ...
        if self.test_cfg.use_nms:
            ...
            nms_predictions = torch.cat(
                [
                    pred_dict["cls_logits"][0, keep_inds].clone().detach(),
                    nms_anchor_params[..., :2],
                    lengths.clone().detach() * self.n_strips,
                    xs.clone().detach() * (self.img_w - 1),
                ],
                dim=-1,
            )  # [N, 77]
            # import pdb; pdb.set_trace()
            # if (scores.device.type == 'cpu'):
            #     nms_predictions,scores, self.test_cfg.nms_thres,self.test_cfg.nms_topk, = torch.tensor(nms_predictions.to(device='cuda')),torch.tensor(scores).to(device='cuda'),torch.tensor(self.test_cfg.nms_thres).to(device='cuda'),torch.tensor(self.test_cfg.nms_topk).to(device='cuda'),
            ...
            # if (keep.device.type == 'cuda' and xs.device.type == 'cpu'):
            #     keep = keep.to(device='cpu')
            ...
            # anchor_params = anchor_params.to(device='cpu')
            xs = xs[keep]
    ...
    def simple_test(self, feats):
        ...
        # lanes, scores = self.get_lanes(pred_dict, as_lanes=True)
        result_dict = {
            "lanes": lanes,
            "scores": scores,
        }

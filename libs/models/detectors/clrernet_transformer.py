...
class CLRerNet_Transformer(SingleStageDetector):
    def __init__(
        self,
        backbone,
        backbone2,
        ...
        """CLRerNet detector."""
        super(CLRerNet_Transformer, self).__init__(
    ...
    def obj_test(self, img,task='',targets=None, **kwargs):
            """
    ...
    def forward(self, img,img2,img_metas,task='',targets=None, **kwargs):
                """
        ...
        #     img.shape[0] == 1 and len(img_metas) == 1
        # ), "Only single-image test is supported."
                    img_metas[0]["batch_input_shape"] = tuple(img2.size()[-2:])

                    # x = self.extract_feat(img)
                    x = self.backbone2(img2)
                    x = self.neck(x)
                    # import pdb; pdb.set_trace()
                    output = self.bbox_head.simple_test(x)
                    result_dict = {
                ...
                if task == "both":
                    ...
                    if self.decoder is not None:
                        decoder_obj = self.decoder(obj_neck, targets)
                    return (decoder_obj, [result_dict])
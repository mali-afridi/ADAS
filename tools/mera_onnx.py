...
# from mmcv import AdamW, CosineAnnealingLrUpd
    ...
    weights2 = torch.load('checkpoint0071.pth')
    # import pdb; pdb.set_trace()

    model.load_state_dict(weights2['model'])
...
# Crop the image
    ...
    for i in range(len(images_list)):
            ...
            frame +=1
            start = time.time()
            obj_out = model.obj_test(img = normalized_image,task='object')
            
            # Export the model
            # Define the export path
            # export_path = "fyp.onnx"

# Convert the model to ONNX format
            # task = "object"
            # img_metas = [0]
            # input_names = ["actual_input"]
            # output_names = ["obj_out"]
            # dummy_input = torch.randn(1, 3, 640, 640).cuda()
            # torch.onnx.export(model,
            #     (dummy_input,dummy_input,img_metas,task),
            #     "fyp.onnx",
            #     verbose=True,
            ...
            # import onnx
            onnx_out = {}
            ort_sess = ort.InferenceSession('fyp.onnx')
            outputs = ort_sess.run(None, {'actual_input': normalized_image.detach().cpu().numpy()})
            onnx_out["pred_logits"] = torch.tensor(outputs[0]).to(device='cuda')
            onnx_out["pred_boxes"] = torch.tensor(outputs[1]).to(device='cuda')
            # import pdb; pdb.set_trace()
            ...
            results = postprocessor(onnx_out, orig_target_sizes)
            ...
            visualize_detections(src=src,preds=None, detections=results,save_path='ali.png')
            # break
            # visualize_detections('.'+images_list[i][:-1], results,save_path='ali.png')
            # import pdb; pdb.set_trace()
